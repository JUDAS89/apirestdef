...
// Conexión inicial a la base de datos postgres para crear la base de datos joyas si no existe
...
const setupDatabase = async () => {
  try {
    ...
    // Conexión a la base de datos joyas para crear las tablas y datos iniciales
    const joyasPool = new Pool({
      user: process.env.DB_USER,
      host: process.env.DB_HOST,
    ...
    const joyasClient = await joyasPool.connect();

    // Leer el script SQL
    const initSql = fs.readFileSync(path.join(__dirname, 'init.sql')).toString();

    // Ejecutar el script SQL
    await joyasClient.query(initSql);
